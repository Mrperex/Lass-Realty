    ...
    return (
        <main className="min-h-screen bg-white pt-28 pb-24">
            ...
            {post.coverImage && (
                <div className="relative w-full h-[50vh] md:h-[60vh] mb-12">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16 max-w-5xl mx-auto">
                        <span className="inline-block bg-gold-500 text-white text-xs font-semibold tracking-wider uppercase px-4 py-1.5 mb-4 font-montserrat">
                            {post.category?.replace('-', ' ')}
                        </span>
                        <h1 className="text-3xl md:text-5xl font-playfair font-medium text-white leading-tight mb-4">
                            {title}
                        </h1>
                        <div className="flex items-center space-x-6 text-white/80 text-sm font-montserrat">
            ...
            <article className="max-w-3xl mx-auto px-6 lg:px-0">
                {!post.coverImage && (
                    <div className="mb-12">
                        <span className="inline-block bg-navy-900 text-white text-xs font-semibold tracking-wider uppercase px-4 py-1.5 mb-6 font-montserrat">
                            {post.category?.replace('-', ' ')}
                        </span>
                        <h1 className="text-3xl md:text-5xl font-playfair font-medium text-navy-900 leading-tight mb-4">
                            {title}
                        </h1>
                        <div className="flex items-center space-x-6 text-gray-500 text-sm font-montserrat">
                            <div className="flex items-center">
                                <Calendar className="w-4 h-4 mr-2" />
                                {new Date(post.publishedAt).toLocaleDateString(getDateLocale(locale), {
                                    year: 'numeric', month: 'long', day: 'numeric'
                ...
                <div
                    className="max-w-none font-montserrat text-gray-700 text-lg leading-relaxed
                ...
                <div className="flex items-center justify-between mt-16 pt-8 border-t border-gray-200">
                    <Link
                        href={`/${locale}/blog`}
                        className="flex items-center text-navy-900 hover:text-gold-500 transition-colors font-montserrat text-sm font-semibold uppercase tracking-wider"
                    >
